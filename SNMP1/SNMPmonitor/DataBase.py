...
class connectorDB:
    ...
    def deleteDevice(self,ip):
        ...
        mariadb_connection.commit()
        mariadb_connection.close()
    def upDateDevice(self,ip,system,location,contact,nick):
        mariadb_connection = mariadb.connect(user=self.user, password=self.password, database=self.database)
        cursor =  mariadb_connection.cursor()
        try:
            cursor.execute("update device set opSystem = %s ,contact = %s,location = %s,nick = %s where ip = %s",(system,contact,location,nick,ip,))
        except mariadb.Error as error:
            print("Error: {}".format(error))           
        mariadb_connection.commit()
        mariadb_connection.close()
